document.addEventListener('DOMContentLoaded', () => {
    ...
    function showGameMessage(text, colorType) {
        ...
        msg.style.color = colorType === 'red' ? 'var(--neon-red)' : (colorType === 'blue' ? 'var(--neon-blue)' : 'var(--neon-green)');
        msg.style.textShadow = `0 0 10px ${msg.style.color}`;
        msg.style.zIndex = '2000';
        msg.style.pointerEvents = 'none';
        msg.style.padding = '10px 20px';
        msg.style.background = 'rgba(0,0,0,0.7)';
        msg.style.border = `1px solid ${msg.style.color}`;
        msg.style.borderRadius = '10px';
        msg.style.animation = 'float-up 1.5s forwards';

        document.querySelector('.battlefield').appendChild(msg);
    ...
    function updateInfoPanel(monster) {
        ...
        const panelPellicle = document.getElementById('panel-pellicle');
        ...
        if (panelImg) {
            ...
            panelImg.src = `Images/${imgName}.png`;
            panelImg.classList.remove('hidden');
        }

        if (panelPellicle) panelPellicle.innerText = `${monster.pellicle} / ${monster.max}`;

        if (panelStatus) {
            panelStatus.innerText = monster.isDead ? "DECEASED" : (monster.pellicle === 0 ? "VULNERABLE" : "ACTIVE");
            panelStatus.style.color = monster.isDead ? "#666" : (monster.pellicle === 0 ? "var(--neon-red)" : "var(--neon-green)");
        }

        if (panelAttack && monster.ability) {
            panelAttack.innerText = monster.ability.attack;
        }

        if (panelPassive && monster.ability) {
            panelPassive.innerText = monster.ability.passive;
        }
    }
    function renderEnemyFormation() {
        const eStats = enemyTeam.map(m => `${m.name}:${m.pellicle}`).join(', ');
        console.log(`[RENDER ENEMY] ${eStats}`);

        const container = document.getElementById('enemy-layer');
        if (!container) return;
    ...
    function spawnPellicleTokens(count) {
        ...
        console.log(`[SPAWN] Pellicle Layer Size: ${layerWidth}x${layerHeight}`);
        ...
        for (let i = 0; i < count; i++) {
            ...
            // DESKTOP FIX: Due to transform: scale(1.3), edges are clipped.
            // We need a larger safe zone on desktop (>1024px).
            const isDesktop = window.innerWidth >= 1024;
            const safeMarginPercent = isDesktop ? 0.2 : 0.05; // 20% margin on desktop, 5% on mobile
            const minPx = isDesktop ? 150 : 40;

            const margin = Math.max(layerWidth * safeMarginPercent, minPx);

            const side = (i % 2 === 0) ? -1 : 1; // -1 Left, 1 Right

            let x, y;
            // Random offset within a safe strip (e.g. between margin and margin+60)
            const xRandom = Math.random() * 60;

            if (side === -1) {
                x = margin + xRandom;
            } else {
                x = layerWidth - margin - xRandom - 70; // 70 is token width
            }

            // Y: Mostly in the bottom half (Player area)
            // Battlefield height is roughly split 40/20/40. 
            // Previous: 0.6 to 0.9. New: 0.55 to 0.85 (Shifted up slightly)
            y = (layerHeight * 0.55) + (Math.random() * layerHeight * 0.3);
    ...
    function runAIAction() {
        ...
        if (attacker.id.includes('cano') && attacker.pellicle >= 3) {
            ...
            for (let i = 0; i < count; i++) {
                setTimeout(() => {
                    if (attacker.isDead || attacker.pellicle <= 0) return; // ATTACKER DIED MID-SEQUENCE (e.g. Reflect)
        ...
        if (attacker.id.includes('lydro') && attacker.pellicle >= 2) {
            ...
            if (useAbility || attacker.pellicle >= 2) {
                ...
                triggerProjectile(attackerIdx, victimIdx, false, () => {
                    ...
                    resolveHit(playerTeam[victimIdx], victimIdx, playerTeam, attacker, attackerIdx, enemyTeam);
                    renderFormation();
                    renderEnemyFormation(); // Update AI in case of reflect
                    setTimeout(() => endTurn(), 1000);
                }, 200, 'hydro'); // Pass special visual type
                return;
            }
        }
    ...
    function getCenterInOverlay(slot) {
        const battlefield = document.querySelector('.battlefield');
        if (!battlefield || !slot) return { x: 0, y: 0 };
        const bRect = battlefield.getBoundingClientRect();
        const sRect = slot.getBoundingClientRect();
        const scale = bRect.width / battlefield.offsetWidth;
        return {
            x: (sRect.left + sRect.width / 2 - bRect.left) / scale,
            y: (sRect.top + sRect.height / 2 - bRect.top) / scale
        };
    }
    ...
    function triggerExplosion(team, index) {
            ...
            neighbors.forEach(nIdx => {
                ...
                if (neighbor && !neighbor.isDead && nIdx < 3) {
                    ...
                    if (neighbor.pellicle > 0) {
                        ...
                        neighbor.pellicle = 0;
                        showGameMessage(`${neighbor.name} is OUT!`, "gray");
                    }
                }
            });
        ...
        }, 800); // 800ms of jitter before pop
    }
    ...
    function addPellicleToEnemyFromDrag(index, tokenId) {
        ...
        // CHECK OVERLOAD (6th point)
        const isCano = monster.id.includes('cano');
        const gainAmount = 1; // Dragging on enemy always adds 1
        ...
        if (!monster.isDead && monster.pellicle < monster.max) {
            pelliclePool--; // Deduct immediately

            const executeEffect = () => {
                monster.pellicle += gainAmount;
                if (isCano) triggerVisualEffect(index, false, 'power-up');
                renderEnemyFormation();
                updatePhaseUI();
                console.log(`Pumping ${monster.name}. Pool: ${pelliclePool}`);
            };

            // ANIMATE TOKEN FLYING TO ENEMY
            if (tokenId) {
                const tokenEl = document.getElementById(tokenId);
                if (tokenEl) {
                    animatePellicleToMonster(tokenEl, index, executeEffect, false); // false = isPlayerAndTarget
                    return;
                }
            }
    ...
    function handleDropEnemySlot(e, enemyIndex) {
        ...
        if (currentPhase === 'REINFORCE') {
            const rawData = e.dataTransfer.getData('text/plain');
            if (isDraggingPellicleFlag || (rawData && rawData.includes('type:pellicle'))) {
                const tokenId = rawData.split('id:')[1] || null;
    ...
    // --- ATTACK LOGIC ---
    function resetReflectFlags() {
        playerTeam.forEach(m => m.hasReflectedThisAction = false);
        enemyTeam.forEach(m => m.hasReflectedThisAction = false);
    }

    function triggerCombatSequence(attackerIndex, victimIndex) {
        resetReflectFlags(); // Start of attack action
        const attacker = playerTeam[attackerIndex];
        ...
        let currentAttackType = 'standard';
        if (attacker.id.includes('nitro')) currentAttackType = 'nitro';
        else if (attacker.id.includes('lydro')) currentAttackType = 'hydro';

        triggerProjectile(attackerIndex, victimIndex, true, () => {
            const isNitroBlast = (currentAttackType === 'nitro');
            updateBattleLog(`${attacker.name} USES ${isNitroBlast ? 'NITRO BLAST' : 'HYDRO SHOT'}!`);
            ...
            if (isNitroBlast) {
                ...
                if (splashIdx !== null) {
                    ...
                    const sourceSlot = getSlotElement(victimIndex, false);
                    const targetSlot = getSlotElement(splashIdx, false);
    ...
    function triggerProjectile(attackerIndex, victimIndex, isPlayerAttacking, onHit, duration = 100, visualType = 'standard') {
        ...
        if (visualType === 'nitro') {
            ...
            const img = document.createElement('img');
            img.src = 'Images/Bullet.png'; // Hidden by CSS but structure kept
            bullet.appendChild(img);
        } else if (visualType === 'osmotic') {
            bullet.classList.add('osmotic-orb');
            // No image needed for orb, but consistent structure is fine
            const img = document.createElement('img');
            img.src = 'Images/Bullet.png';
            bullet.appendChild(img);
        } else {
        ...
        if (visualType === 'hydro') {
            // For streams with 'center left' origin, start point IS the source center
        ...
        // Hydro/Osmotic use rotation from the start
        ...
        // 3. SET TRANSITION & ANIMATE
        bullet.style.transition = `transform ${duration / 1000}s linear`;

        if (visualType === 'hydro' || visualType === 'osmotic') {
            // Move along the rotated X axis
            let travelDist = distance;
            if (visualType === 'hydro') travelDist *= 1.25; // Penetrate slightly further
            bullet.style.transform = `rotate(${angle}deg) translate(${travelDist}px, 0px)`;
        } else {
            // Standard slide for round projectiles
    ...
    function triggerReflectVisual(sourceSlot, targetSlot, onHit) {
        if (!sourceSlot || !targetSlot) {
            if (onHit) onHit();
            return;
        }
        ...
        // Calculate rotation to face attacker
    ...
    function resolveHit(victim, victimIndex, team, attacker = null, attackerIndex = -1, attackerTeam = null, hitType = 'heavy', visualEffectOverride = null) {
        ...
        if (!visualEffectOverride) triggerScreenShake(); // Only shake on main hits
        ...
        if (victim.pellicle > 0) {
            console.log(`[DAMAGE] ${victim.name} (Team ${team === playerTeam ? 'Player' : 'Enemy'}) takes damage from ${attacker ? attacker.name : 'Unknown'}`);
            victim.pellicle -= 1;
        ...
        if (reflectDamage) {
            ...
            const isVictimPlayer = (team === playerTeam);
            const isAttackerPlayer = (attackerTeam === playerTeam);

            // 1. STANDALONE BURST (Reliable - won't be cleared by re-render)
            triggerBurstEffect(victimIndex, isVictimPlayer, 'reflect-burst');

            // 2. PROJECTILE (Spike back to attacker)
            const sourceSlot = getSlotElement(victimIndex, isVictimPlayer);
            const targetSlot = getSlotElement(attackerIndex, isAttackerPlayer);

            triggerReflectVisual(sourceSlot, targetSlot, () => {
                // 3. APPLY DAMAGE ON HIT
                if (attackerIndex !== -1 && attacker) {
                    triggerVisualEffect(attackerIndex, isAttackerPlayer, 'hit-flash');
                }
    ...
    function showGameOverEffect(message, color) {
        // Change target to GAME BOARD to blur everything (Battlefield + Info Panel)
        const board = document.getElementById('game-board');
        const overlay = document.getElementById('game-over-overlay');
        const overlayMsg = document.getElementById('game-over-msg');

        // 1. APPLY BLUR TO BOARD
        if (board) board.classList.add('blur-focus');
        ...
        // HOVER FOR INFO PANEL (DESKTOP)
        // Note: monsterEl and monster are not defined in this scope.
        // This listener will cause a ReferenceError if uncommented without proper context.
        // monsterEl.addEventListener('mouseenter', () => {
        ...
        setTimeout(() => {
            ...
            if (board) board.classList.remove('blur-focus');
    ...
    function handleMonsterClick(index) {
        ...
        if (selectedAbilitySourceIndex !== null) {
            ...
            // EXECUTE TRANSFER with VISUAL
            // 1. DEDUCT from Source Immediately (to prevent spam)
            source.pellicle--;
            renderFormation();

            // 2. TRIGGER VISUAL (Osmotic Orb)
            triggerProjectile(selectedAbilitySourceIndex, index, true, () => {
                // 3. APPLY TO TARGET ON HIT
                target.pellicle++;
                ...
                renderFormation();
                updateInfoPanel(target); // Force update panel if hovering target
            }, 600, 'osmotic'); // Slow speed (600ms) for gentle feel

            // Clear instruction after use
            selectedAbilitySourceIndex = null; // Clear immediately to allow next pick
            updatePhaseUI();
            renderFormation();
        }
    }
    ...
    function showTooltip(monster, x, y) {
        if (!tooltip || !monster || !monster.name) return;
    ...
    function triggerBurstEffect(index, isPlayer, className) {
        const slot = getSlotElement(index, isPlayer);
        const overlay = document.getElementById('effects-overlay');
        if (!slot || !overlay) return;
        ...
        burst.classList.add('burst-effect', className);
    ...
    // DEBUG: TEST SCENARIO
    window.setupTestScenario = function () {
        console.log("SETTING UP TEST SCENARIO");
        // 1. Maximize Player Lydrosome (Index 1)
        playerTeam[1].pellicle = 5;

        // 2. Reset Enemy Team
        enemyTeam.forEach(m => m.damage = 0);
        enemyTeam[0].pellicle = 2; // Nitro
        enemyTeam[1].pellicle = 2; // Lydro
        enemyTeam[2].pellicle = 2; // Cano

        renderFormation();
        renderEnemyFormation();
        updatePhaseUI();
        console.log("TEST SCENARIO READY: Lydrosome (5P) vs Full Enemy Team");
    };
